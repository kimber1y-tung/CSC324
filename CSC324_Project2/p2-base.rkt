   ...
   [(fresh (first rest ids body bin br new-env types restype argtypes fill args)
           ...
           (conde
            [(pairo body)
             ...
             (== body (cons bin br))
             (typeo bin new-env types)
             (== types (list fill restype))
             (helpero ids body env argtypes)
             (== type (list argtypes restype))]
            [(fresh (idtypes newenv)
                    (update-envo ids env newenv)
                    (fill-idso ids newenv idtypes)
                    (conde
                     [(symbolo body)
                      (fresh (restype)
...
(define (helpero ids body env types)
  (conde
   ...
   [(fresh (first rest btypes bargs bres newenv fid rid ft rt)
           ...
           (== types (cons ft rt))
           (== btypes (cons bargs (cons bres '())))
           (typeo first env btypes)
           (typeo first newenv btypes)
           (typeo body newenv bres)
           (lookupo newenv fid ft)
           (helpero rid body env rt))]))
